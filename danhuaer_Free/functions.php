...
function post_thumbnail(){
	...
	if($cnt>0){
		...
		$post_img = '<a class="img" href="'.$link.'" title="'.$title.'"><img src="'.get_bloginfo('template_url').'/timthumb.php?src='.$post_img_src.'&amp;w=200&amp;zc=1" alt="'.$title.'"/><div class="g">'.mb_strimwidth(strip_tags($post->post_content),0,300,"...").'</div></a>';
		echo $post_img;
	}else{
		echo '<a class="noimg" href="'.$link.'" title="'.$title.'"><span>'.$title.'</span>'.mb_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 500,"...").'</a>';
	}
}
//缩略图
function catch_that_image() {
	...
	if ( has_post_thumbnail($post->ID) ) {
		...
		$first_img = '/images/rand/'.$tu.'.jpeg'; //如果文章没有图片则调用这个默认图片作为缩略图
		}
	}
	return $first_img;
}
