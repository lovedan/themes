...
$attachDir='wp-content/uploads/tougao';//上传文件保存路径，结尾不要带/
...
$err = "";
$msg = "''";
$tempPath=$_SERVER["DOCUMENT_ROOT"].'/'.$attachDir.'/'.date("YmdHis").mt_rand(10000,99999).'.tmp';
if(!is_dir($_SERVER["DOCUMENT_ROOT"].'/'.$attachDir))
	{
		@create_folders($_SERVER["DOCUMENT_ROOT"].'/'.$attachDir);
			}
$localName='';
...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			{
				@create_folders($_SERVER["DOCUMENT_ROOT"].'/'.$attachDir);
				@fclose(fopen($_SERVER["DOCUMENT_ROOT"].'/'.$attachDir.'/index.htm', 'w'));
			}
			PHP_VERSION < '4.2.0' && mt_srand((double)microtime() * 1000000);
			$newFilename=date("YmdHis").mt_rand(1000,9999).'.'.$extension;
			$targetPath = $_SERVER["DOCUMENT_ROOT"].'/'.$attachDir.'/'.$newFilename;
