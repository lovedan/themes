...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		err( __('<strong>ERROR</strong>: please fill the required fields (name, email).') ); // 将 wp_die 改为错误提示
	elseif ( !is_email($comment_author_email))
		err( __('<strong>ERROR</strong>: please enter a valid email address.') );// 将 wp_die 改为错误提示
}

if ( '' == $comment_content )
	err( __('<strong>ERROR</strong>: please type a comment.') ); // 将 wp_die 改为错误提示
...
?>
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="commentmeta"><?php echo get_avatar( $comment, 48 ); ?></div>
		<?php if ( $comment->comment_approved == '0' ) : ?>
		<em><font color=Red>您的评论需要管理员审核...</font></em>
		<?php endif; ?>
	...
	<div class="vcard"><?php printf( __( '%s <span class="says">says:</span>'), get_comment_author_link() ); ?></div>
		<?php comment_text(); ?>
	</div>