...
//获取文章分类顶级栏目的别名
function get_category_root_link($this_cat)
{
$this_category = get_category($this_cat);  // 取得当前分类
/* while($this_category->category_parent)   // 若当前分类有上级分类时，循环
 {
  $this_category = get_category($this_category->category_parent);   // 将当前分类设为上级分类（往上爬）
   //$cat_ID=$this_category->term_id;
  //$this_cat = get_category($cat_ID);     // 根据分类ID获取分类
  //$slug = $this_cat->slug;   // 返回根分类的别名
  //$slugs .= $slug."/";
 } */
  $cat_ID=$this_category->term_id;
  $this_cat = get_category_link($cat_ID);     // 根据分类ID获取分类
  return $this_cat;  // 返回根分类的别名
}
function get_category_root_ico($this_cat)
{
$this_category = get_category($this_cat);  // 取得当前分类
while($this_category->category_parent)   // 若当前分类有上级分类时，循环
 {
